...
#The actual access token is not included here since this is a public repository.

def extract_project_PRs(project_full_name):
    df_PRs = pd.DataFrame()
    while True:
        try:
            g = Github(access_token, retry=10, timeout=15, per_page=500)
            print(f'Extracting data from {project_full_name} repo')
            repo = g.get_repo(project_full_name)
            PRs_list = repo.get_pulls(sort='created')
            ...
            continue
        break
    df_PRs.to_csv('Dataset4b.csv', sep=',', encoding='utf-8', index=True)

extract_project_PRs('google-research/bert')
